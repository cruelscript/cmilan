...
public:
  ...
  // Конструктор для инструкций с одним аргументом
  Command(Instruction instruction, float arg)
    : instruction_(instruction), arg_(0), farg_(arg)
  {
    flag_ = true;
  }
...
public:
  ...
  void emit(Instruction instruction, float farg);
  ...
  void emitAt(int address, Instruction instruction, float arg);
